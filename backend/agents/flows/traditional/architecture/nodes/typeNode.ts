...
export async function typeNode(state: T_Graph) {
  ...
  const generatePromises = targetFiles.map(async (fileNode) => {
    ...
    if (!modelDef) {
      throw new Error(
        `TypeNode: Model definition not found for ${modelId}`,
      );
    }
...
`;
    ...
    try {
      ...
      throw new Error(
        `Type generation failed for ${fileNode.path} after retries: ${message}`,
        { cause: error },
      );
    }
  });
  ...
  console.log(`--- Type Generation End (Generated ${results.length}) ---`);
