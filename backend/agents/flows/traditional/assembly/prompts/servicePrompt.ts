...
【输出目标】
生成符合 \`ServiceSchema\` 的 JSON 对象。最终结果必须是对象，而不是裸数组。

【结构化输出契约（必须严格遵守）】
- 只能返回合法 JSON，不要返回 Markdown 代码块、解释文字或额外字段。
- 顶层结构必须是：\`{ "files": [...] }\`。
- \`files\` 必须是数组；每个服务文件对象必须同时包含 \`path\`、\`content\`、\`description\` 三个字段。
- 每个字段都必须是非空字符串，不能省略 \`description\`。
- 当前调用针对一个领域模型，\`files\` 数组必须恰好包含一个服务文件。
- 服务文件路径必须以 \`/services/\` 开头并以 \`.ts\` 结尾。
- 服务文件名必须使用小驼峰命名，例如模型 \`Project\` 对应 \`/services/projectService.ts\`，模型 \`TeamMember\` 对应 \`/services/teamMemberService.ts\`。
  ...
  "files": [
    {
      ...
      "content": "export function getNovelById(id: string): Novel | undefined { return MOCK_NOVELS.find(novel => novel.id === id); }",
      "description": "提供小说详情和小说列表查询能力"
    }
  ]
}
...
【服务代码逻辑示例（仅参考 content 字段，不能直接作为最终 JSON 输出）】
...
- 不要生成任何注释，只生成纯净代码
- 最终必须返回 \`{ "files": [{ "path": "...", "content": "...", "description": "..." }] }\`
- 不允许返回裸数组，也不允许省略 \`description\`
- 文件名使用小驼峰，不要使用模型名首字母大写的 PascalCase 文件名
${JSON_SAFETY_PROMPT}
`;
